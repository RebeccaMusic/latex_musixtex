...
\usepackage[chorded]{songs} % guitar chords
...
\usepackage{etex} % for "songs" package
\noversenumbers % for "songs" package
...
Although I didn't try out the example given, I found another package songs\cite{tablature} to support guitar chords.
...
\begin{music}
\startextract
\stafftopmarg=50\Interligne%
\zchar{18}{\gtab{G}{0554XX}}
\hardlyrics{clair}%
\notes\zsong{\thelyrics}\wh g\en% 
\notes\tinynotesize\wh{gg}\relax\en
\endextract
\end{music}
\vspace{20pt}
...
In the preamble input 3 sentences:
\begin{verbatim}
\usepackage[chorded]{songs}
\usepackage{etex} % for song package
\noversenumbers % for "songs" package
\end{verbatim}

But I must put \verb"\usepackage[chorded]{songs}" before \verb"\usepackage{musixtex}", otherwise compilation fails. The order matters.
